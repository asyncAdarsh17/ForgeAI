...
import { exportFullHistory } from "../src/storage/exporter.js";
import { readFileContent } from "../src/storage/fileReader.js";
import { analyzeFolder } from "../src/features/analyze.js";
import say from "say";
...
while (true) {
  ...
  else if (choice.includes("Summarize Text")) {
    await summarize();
  }
  ...
  else if (choice.includes("Debug Errors")) {
    await debugCode();
  }
  ...
  else if (choice.includes("Analyze Project")) {
    const { path } = await inquirer.prompt([
  ...
  await inquirer.prompt([
    { type: "input", name: "c", message: "Press Enter to continue..." }
  ]);
